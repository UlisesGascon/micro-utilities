...
export function updateOrCreateSegment(settings: Settings): string {
  ...
  if (
    [original, replacementSegment, startTag, endTag].some(
      (prop) => typeof prop !== "string"
    )
  ) {
    throw new Error(
      "settings must be an object with the following properties as string values: original, replacementSegment, startTag, endTag"
    );
  }
  ...
  if (
    ...
    (endTagIndex === -1 && startTagIndex > -1)
  ) {
    throw new Error(
      `Only one tag found in the content, expected to find both '${startTag}' and '${endTag}'`
    );
  }
  ...
  if (startTagIndex > endTagIndex) {
    throw new Error(
      `The tags are not in the correct order, expected to find '${startTag}' before '${endTag}'`
    );
  }
