...
export function simplifyObject(
  baseObj: OpenObjectType,
  operation: simplifyPayloadType
): OpenObjectType {
  ...
  if (Array.isArray(operation.include) && Array.isArray(operation.exclude))
    throw new Error("operation must have either include or exclude");
  if (!Array.isArray(operation.include) && !Array.isArray(operation.exclude))
    throw new Error("operation must have either include or exclude");

  if (Array.isArray(operation.include) && operation.include.length === 0)
    throw new Error("operation.include must be an array with properties");
  if (Array.isArray(operation.exclude) && operation.exclude.length === 0)
    throw new Error("operation.exclude must be an array with properties");
  ...
  Object.keys(baseObj).forEach((key) => {
    if (
      (include.length > 0 && include.includes(key)) ||
      (exclude.length > 0 && !exclude.includes(key))
    ) {
