...
export function softAssign (obj:OpenObjectType, keys:string[], value:any):void {
  if(!isObject(obj)) throw new Error('obj must be an object')
  if(!Array.isArray(keys) || (keys.length === 0)) throw new Error('keys must be an Array with at least one element')
